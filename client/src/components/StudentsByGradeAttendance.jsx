...
const StudentsByGradeAttendance = () => {
    ...
    const getAttendanceData = async () => {
      const response = await fetch(`http://localhost:3003/attendance/${gradeId}/${date}`, {
        method: "GET",
        headers: {Authorization: `Bearer ${token}`},
      });
      const data = await response.json();
      ...
      try {
        ...
        const savedAttendanceResponse = await fetch(
          "http://localhost:3003/attendance/registerAttendance",
          {
            ...
            body: JSON.stringify({date: date, attendanceData: attendanceData, period: new Date().getFullYear(), gradeId: gradeId}),
          }
        )
